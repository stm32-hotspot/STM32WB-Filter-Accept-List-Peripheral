...
{
  10, AD_TYPE_COMPLETE_LOCAL_NAME, 'W', 'B', '5', '5', 'w', 'l', 'i', 's', 't',  /* Complete name */
...
void APP_BLE_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1<<CFG_TASK_BLE_CLEAR_ID, UTIL_SEQ_RFU, ble_clear_db);
...
/* USER CODE BEGIN FD*/
void app_ble_pairing_request(void)
{
  tBleStatus ret;
...
static void adv_start(void)
{
  ...
  int8_t bonded_total = ble_count_bonded_devices();
  
  if(bonded_total > 0)
  {    
    status = aci_gap_set_undirected_connectable(CFG_FAST_CONN_ADV_INTERVAL_MIN,
                                                CFG_FAST_CONN_ADV_INTERVAL_MAX,
                                                CFG_BLE_ADDRESS_TYPE,
                                                HCI_ADV_FILTER_WHITELIST_SCAN_CONNECT);
    APP_DBG_MSG("\taci_gap_set_undirected_connectable = 0x%02X\r\n", status);
    
    status |= aci_gap_update_adv_data(sizeof(a_AdvData), (uint8_t*) a_AdvData);
    APP_DBG_MSG("\taci_gap_update_adv_data = 0x%02X\r\n", status);
    
    APP_DBG_MSG("\tADV with whitelist = %s\r\n", status == BLE_STATUS_SUCCESS ? "Success" : "FAILED!");
  }
  else
  {
    Adv_Request(APP_BLE_FAST_ADV); // undirected ADV without whitelist
  }
}
...
static void ble_clear_db(void)
{
  ...
  ret = aci_gap_set_non_discoverable(); // STOP advertising if advertising
  APP_DBG_MSG("aci_gap_set_non_discoverable() : 0x%02X\t%s\t\n",
              ret,
              ret == BLE_STATUS_SUCCESS ? "successfully stopped advertising" : "was not advertising");
  
  ret = aci_gap_clear_security_db();
  APP_DBG_MSG("aci_gap_clear_security_db() : 0x%02X\tto clear the bonding table - all bonded devices are removed\r\n", ret);
  if(ret != BLE_STATUS_SUCCESS)
  {
    Error_Handler();
  }
...
static int8_t ble_count_bonded_devices(void)
{
  ...
  List_Entry_t device_accept[16];
  
  tBleStatus ret = aci_gap_configure_filter_accept_list();
  APP_DBG_MSG("\taci_gap_configure_filter_accept_list() status = 0x%02X\r\n", ret);
  ...
  if(BLE_STATUS_SUCCESS == ret)
  {
    ...
    else
    {
      for(uint8_t k = 0; k < total; k++)
      {
        ...
        device_accept[k].Address_Type = devices[k].Address_Type;
        device_accept[k].Address[5] = devices[k].Address[5];
        device_accept[k].Address[4] = devices[k].Address[4];
        device_accept[k].Address[3] = devices[k].Address[3];
        device_accept[k].Address[2] = devices[k].Address[2];
        device_accept[k].Address[1] = devices[k].Address[1];
        device_accept[k].Address[0] = devices[k].Address[0];
        
        // Append to both resolving list and Filter Accept List
        ret = aci_gap_add_devices_to_list(1, &device_accept[k], 0x04);
        APP_DBG_MSG("aci_gap_add_devices_to_list() status = 0x%02X\r\n", ret);
      }
    }
  }
  else
